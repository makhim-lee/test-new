...
#define MCU_RS_PIN  8 //arduion D8
#define MCU_EN_PIN  9 //arduion D9

#define lcd_rs   MCU_RS_PIN
#define lcd_rw   0         //NC = gnd
...
#define        DIS_4_HOME        (lcd_cmd(0xd4))
  ...
  #define        bit_4_if          (lcd_cmd(0x28)) // 2018, 5, 12 
#elif LCD_IF_8_Bit
  #define        FUNC_SET_8BIT     (lcd_cmd(0x38))
  #define        bit_8_if          (lcd_cmd(0x38)) // 2018, 5, 12 
#elif LCD_IF_I2C
  // Nest Ver
#endif
...
#elif LCD_PC
  #define LCD_PORT GPIOC//arm proc
#elif LCD_PD
  #define LCD_PORT PORTD 
// #elif Arduino
//   #define LCD_PORT(data) d_out(data)  //arduino 
#endif
...
{
  // user data display
	d_out(lcd_rs, 1); // User data Display rs = 1;
...
void lcd_gotoxy_str(char x, char y, unsigned char *buf)
{
 // point set and print
 lcd_gotoxy(x,y);
 lcd_str(buf);
...
void user_font_rd_8()
 {
  // 먼저 쓰고 그다음 읽기
   lcd_send(0x00);  // 0x40 = ST_ADR 1 == 0
   lcd_send(0x01);  // 0x48 = ST_ADR 2 == 1
   lcd_send(0x02);  // == 2
   lcd_send(0x03);  // == 3
   lcd_send(0x04);  // == 4
   lcd_send(0x05);  // == 5
