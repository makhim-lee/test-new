...
class Array{
private:
    static const int ARRAY_SIZE = 100; // 전역변수인데 클레스에서관련돼서만 쓰인다는 의미 // public에 할당할경우 클레스이름을 통해서 전역에서 접근 할 수 있게됨
...
public:
    ...
    explicit Array(int size = Array::ARRAY_SIZE);   //명시적인 Array arr3 = 100 xxxx 형변환을 막음 
    Array(const int *pArr, int size = Array::ARRAY_SIZE);
